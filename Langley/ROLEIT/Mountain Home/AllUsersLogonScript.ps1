...
$Base = $adobj.Properties.l
$Org = $adobj.Properties.o
...
#================================
#Create Favorites
Copy-Item '\\area52.afnoapps.usaf.mil\mountain_home_afb\Logon_Scripts\Scripts\Support Files\Favorites\*' -Destination $([Environment]::GetFolderPath('Favorites','None')) -Recurse -Force

#Map Shared Drives
$Drivemappings=Import-Csv '\\area52.afnoapps.usaf.mil\mountain_home_afb\Logon_Scripts\Scripts\Support Files\SecurityGroupDrives.csv'
Foreach($group in $groups)
{
    if ($group -in $Drivemappings.'Security Group'){
        $mapdrive=$Drivemappings | Where-Object {$_.'Security Group' -eq $group}
        $mapdrive.ForEach{
            Map-NetworkDrive -DriveLetter "$($_.PDL.trim(':'))" -Path "$($_.'Network Path')"
            }
    }
}

#Set Home Page
Set-IEHomePage "http://www.mountainhome.af.mil"

#Updates
start 'C:\Program Files\McAfee\Agent\mcupdater.exe' -ArgumentList "/update /quiet"

#Logon Information
$logpath= "\\gyzh-fs-fas03p\DomainLogs$\Domain.txt"
...
"$(Get-Date -Format 'MM/dd/yyyy hh:mm:ss tt'), $env:COMPUTERNAME, $env:Username, $SN, $IP, $Org, $Base, $MAC" >> $logpath
