...
Map-NetworkDrive M "\\pnqs-cifs-001\PUBLIC"

#Change Theme for AETC Background Gen Webb
PowerShell.exe -ExecutionPolicy Bypass -File \\area52.afnoapps.usaf.mil\Maxwell_AFB\Logon_Scripts\Source_Files\PowerShell\SetTheme.ps1

#SAPR Popup
$Date=Get-Date
if($Date.Month -eq 7){
    if($Date.Day -eq 8){Invoke-Expression "\\area52.afnoapps.usaf.mil\Maxwell_AFB\Logon_Scripts\SAPR.2013.07.08.hta"}
    if($Date.Day -eq 15){Invoke-Expression "\\area52.afnoapps.usaf.mil\Maxwell_AFB\Logon_Scripts\SAPR.2013.07.15.hta"}
    if($Date.Day -eq 22){Invoke-Expression "\\area52.afnoapps.usaf.mil\Maxwell_AFB\Logon_Scripts\SAPR.2013.07.22.hta"}
    }
