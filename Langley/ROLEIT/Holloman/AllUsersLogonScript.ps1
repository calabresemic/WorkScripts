...
Run-Slides -File '\\kwrd-fs-dfsv\Holloman_DFS\49 CS\Electronic Bulletin Board\Slides.pptx'

#Logon Record
$SerialNumber = (Get-WmiObject win32_bios).SerialNumber
$NetComp="\\kwrd-fs-appv\DFSv\Computers"
$NetUser="\\kwrd-fs-appv\DFSv\Users"

if( (Test-Path $NetComp) -and (Test-Path $NetUser) ) {
    "$env:COMPUTERNAME, $SerialNumber, $(Get-Date)" >> "$NetComp\$env:COMPUTERNAME;$SerialNumber.txt"
    "$env:Username, $(Get-Date)" >> "$NetUser\$env:username.txt"
}
