...
<# Revision History
 ...
 21 Jan 2021 - Michael Calabrese (1468714589) - fixed Ada's mess
#>
...
Function Map-NextAvailableDrive($path)
{
$driveletter=65..90|foreach-object{[char]$_} | ?{ !(Get-PSDrive $_ -ErrorAction SilentlyContinue) } | select -First 1
New-PSDrive -name $driveletter -psprovider FileSystem -root $path -Scope Global
}
...
Create-Shortcut -name "36 Wing SharePoint.lnk" -target "https://andersen.eis.pacaf.af.mil/Pages/default.aspx" -icon "\\area52.afnoapps.usaf.mil\andersen_afb\Logon_Scripts\36WG.ico"

#Filtering by groups also enabled here, though it should only be done if groups exist outside one specific unit
#================================

If("Andersen AFB All Admin" -in $groups)
{
Map-NetworkDrive H "\\52ajjy-fs-001\Admin"
}
