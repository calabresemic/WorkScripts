...
#================================
Map-NetworkDrive O "\\XHZG-FS-01P\Public"
Map-NetworkDrive I "\\XHZG-FS-02P\ERM"

robocopy \\XHZG-FS-01P\Public\PERMANENT\138_MSG\138_CF\138th_Fighter_Wing_Links "$([Environment]::GetFolderPath('Favorites'))\138th_Fighter_Wing_Links\" /MIR
#Filtering by groups also enabled here, though it should only be done if groups exist outside one specific unit
#================================
If("GLS_Tulsa_CFP-CSA" -in $groups)
{
Map-NetworkDrive Z "\\XHZG-FS-01P\CSA"
}

If("138 FW 138UPRG" -in $groups)
{
Map-NetworkDrive H "\\XHZG-FS-01P\UPRG"
}

If("138 FW 138FEP" -in $groups)
{
Map-NetworkDrive S "\\XHZG-FS-01P\FEP"
}

If("138 FW 138PA" -in $groups)
{
Map-NetworkDrive V "\\138FW-FS-03\VI"
}
