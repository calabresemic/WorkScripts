        ...
        :rulecheck switch ($STIGs[$STIG_index].STIG_DATA.ATTRIBUTE_DATA[$Rule_ID_index]) {
            ...
            "SV-42341r1_rule" {
                ...
                foreach($file in $machineConfig | Where-Object {$_.length -ge 1}){
                    $files+=Select-String -Path $file -Pattern '*channel ref="tcp"*' -SimpleMatch}
                if($files -eq $null){$ActualStatus = "NotAFinding"}
                else{$ActualStatus = "Open"
                    foreach($thing in $files.path){$output+="$thing`n"}
                    $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "Manually check the following files: `n$output"}
                }
            #V-7055
            ...
            "SV-40966r1_rule" {
                $ManuallyVerify += $STIGs[$STIG_index].STIG_DATA.ATTRIBUTE_DATA[$Vuln_Num_Index]
                }
            ...
            #V-97527
            #Microsoft Internet Explorer 11 Security Technical Implementation Guide :: Version 1, Release: 18 Benchmark Date: 25 Oct 2019
...
"@
            ...
            "SV-52213r1_rule" {
                ...
                catch {$ManuallyVerify += $STIGs[$STIG_index].STIG_DATA.ATTRIBUTE_DATA[$Vuln_Num_Index]}
                }
...
"@.Split("`n") | foreach {$_.trim()}
            ...
            "SV-52234r4_rule" {
                ...
                elseif (($auditpol -match "Computer Account Management") -match "Success and Failure") {$ActualStatus = "NotAFinding"}
                else {$ActualStatus = "Open"}
                }
            ...
            "SV-51579r1_rule" {
                ...
                $value = ($GPResult -match "Minimum password length").replace("<td>",";").split(";")[2].split("<")[0].split(" ")[0]
                if ($Value -ge "15") {
...
"@.split("`n") | Foreach {$_.trim()}
            ...
            "SV-32264r1_rule" { 
                #Check all non system shares to see if theres Everyone permission with allow.  If not, it's sure constrained permissions
                if ($serverrole -eq 2){
                    $NonDefaultShares = Get-SmbShare | where {$_.name -ne "ADMIN$" -and $_.name -ne "C$" -and $_.name -ne "IPC$" -and $_.name -ne "NETLOGON" -and $_.name -ne "SYSVOL"}
                    }
                else {$NonDefaultShares = Get-SmbShare | where {$_.name -ne "ADMIN$" -and $_.name -ne "C$" -and $_.name -ne "IPC$"}
                    }
                foreach ($share in $NonDefaultShares) {
                    $SharePerms = Get-SmbShareAccess $share.name
                    if ($SharePerms | where {$_.AccountName -match "Everyone"} -and $_.AccessControlType -eq "Allow") {$ActualStatus = "Open"}
            ...
            "SV-101879r1_rule" { 
                ...
                if ($krbtgtSet -eq $null) {$ManuallyVerify += $STIGs[$STIG_index].STIG_DATA.ATTRIBUTE_DATA[$Vuln_Num_Index]}
            ...
            #V-76713
            #IIS 8.5 Server Security Technical Implementation Guide :: Version 1, Release: 9 Benchmark Date: 25 Oct 2019
            ...
            "SV-91455r2_rule" { 
                #Checks the Reg Keys.. in an if if if if loop but I don't see you coding this
                if((Check-RegKeyValue "HKLM\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.1\Server" -regValueName DisabledByDefault) -eq 0){
                    if((Check-RegKeyValue "HKLM\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.2\Server" -regValueName DisabledByDefault) -eq 0){
                        if((Check-RegKeyValue "HKLM\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.0\Server" -regValueName DisabledByDefault) -eq 1){
                            if((Check-RegKeyValue "HKLM\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.0\Server" -regValueName Enabled) -eq 0){
                                if((Check-RegKeyValue "HKLM\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\SSL 2.0\Server" -regValueName DisabledByDefault) -eq 1){
                                    if((Check-RegKeyValue "HKLM\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\SSL 2.0\Server" -regValueName Enabled) -eq 0){
                                        if((Check-RegKeyValue "HKLM\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\SSL 3.0\Server" -regValueName DisabledByDefault) -eq 1){
                                            if((Check-RegKeyValue "HKLM\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\SSL 3.0\Server" -regValueName Enabled) -eq 0){
                                                $ActualStatus = "NotAFinding"}
                                                }
                                            }
                                        }
            ...
            #V-36431
            #Active Directory Domain Security Technical Implementation Guide (STIG) :: Version 2, Release: 13 Benchmark Date: 26 Apr 2019
            ...
            #V-42531
            #McAfee VirusScan 8.8 Managed Client STIG :: Version 5, Release: 21 Benchmark Date: 25 Oct 2019
            ...
            "SV-55259r5_rule" { 
                #Can be checked but if open is POAMed
                $Value = Check-RegKeyValue "HKLM\Software\Wow6432Node\McAfee\SystemCore\VSCore\On Access Scanner\McShield\Configuration\Default\" "NumExcludeItems"
                if ($Value -eq "0") {$ActualStatus = "NotAFinding"}
                else {$ActualStatus = "Open"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "This vulnerability is currently POAMed. A copy of the POAM is located on SIPR in the following location: \\muhj-fs-001\CYOD\11--Cyber 365\07--POAM & MFRs"}
                }
            ...
            "SV-73009r5_rule" { 
                #Can't be checked
                $ActualStatus = "Not_Applicable"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "This is NA, we use forwarders"
                }
                #V-58581
            #Microsoft Windows 2012 Server Domain Name System Security Technical Implementation Guide :: Version 1, Release: 12 Benchmark Date: 26 Jul 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-73051r4_rule" { 
                ...
                $oldrecords+=$CNames | Where-Object {$_.Timestamp -lt ((Get-Date).AddMonths(-6))}
                }
            ...
            "SV-7438r3_rule" { 
                #Can't be checked
                $ActualStatus = "NotAFinding"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "REQ000000382981 /This registry key needs to be created "
                }
                #V-7061
            #Microsoft DotNet Framework 4.0 STIG :: Version 1, Release: 9 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-7453r3_rule" { 
                #Can't be checked
                $ActualStatus = "NotAFinding"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "We are only using HTTP, TCP and IPC for remoting channels"
                }
                #V-18395
            #Microsoft DotNet Framework 4.0 STIG :: Version 1, Release: 9 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-40977r3_rule" { 
                #Can't be checked
                $ActualStatus = "Open"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "REQ000000382928/Setting needs to be configured "
                }
                #V-30937
            #Microsoft DotNet Framework 4.0 STIG :: Version 1, Release: 9 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-41010r1_rule" { 
                #Can't be checked
                $ActualStatus = "NotAFinding"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "We do not have custom configurations that would allow us to set this setting"
                }
                #V-30972
            #Microsoft DotNet Framework 4.0 STIG :: Version 1, Release: 9 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-42341r2_rule" { 
                #Can't be checked
                $ActualStatus = "NotAFinding"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "We use HTTP, TCP and IPC for remoting"
                }
                #V-46473
            #Microsoft Internet Explorer 11 Security Technical Implementation Guide :: Version 1, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            "SV-59337r8_rule" { 
                #Can't be checked
                $ActualStatus = "Open"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "REQ000000382900/This setting needs to be configured"
                }
                #V-46797
            #Microsoft Internet Explorer 11 Security Technical Implementation Guide :: Version 1, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            "SV-59663r1_rule" { 
                #Can't be checked
                $ActualStatus = "Open"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "REQ000000382910/This setting needs to be configured"
                }
                #V-46799
            #Microsoft Internet Explorer 11 Security Technical Implementation Guide :: Version 1, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            "SV-59665r1_rule" { 
                #Can't be checked
                $ActualStatus = "Open"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "REQ000000382926/This setting needs to be configured"
                }
                #V-97527
            #Microsoft Internet Explorer 11 Security Technical Implementation Guide :: Version 1, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            "SV-106631r1_rule" { 
                #Can't be checked
                $ActualStatus = "Open"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "REQ000000382927/This setting needs to be configured"
                }
                #V-1070
            #Microsoft DotNet Framework 4.0 STIG :: Version 1, Release: 9 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-52841r2_rule" { 
                #Can't be checked
                $ActualStatus = "NotAFinding"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "AFNET is backed up on a regular basis"
                }
                #V-1112
            #Microsoft Internet Explorer 11 Security Technical Implementation Guide :: Version 1, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-52856r1_rule" { 
                #Can't be checked
                $ActualStatus = "Open"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "REQ000000382959/This setting needs to be configured"
                }
                #V-1115
            #Microsoft Internet Explorer 11 Security Technical Implementation Guide :: Version 1, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-52213r2_rule" { 
                #Can't be checked
                $ActualStatus = "Not_Applicable"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "There are no printers configured and this is a NA"
                }
                #V-1168
            ...
            "SV-52156r2_rule" { 
                #Can't be checked
                $ActualStatus = "Open"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "MFR/There are around 15-20 service accounts in this group."
                }
                #V-2907
            #Windows Server 2012/2012 R2 Domain Controller Security Technical Implementation Guide :: Version 2, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-52881r3_rule" { 
                #Can't be checked
                $ActualStatus = "NotAFinding"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "All configurations are compliant with STIG "
                }
                #V-3487
            #Windows Server 2012/2012 R2 Domain Controller Security Technical Implementation Guide :: Version 2, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            "SV-52218r2_rule" { 
                #Can't be checked
                $ActualStatus = "Open"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "This vulnerability is currently MFR. A copy of the POAM is located on SIPR in the following location: \\muhj-fs-001\CYOD\11--Cyber 365\07--POAM & MFRs "
                }
                #V-4407
            #Windows Server 2012/2012 R2 Domain Controller Security Technical Implementation Guide :: Version 2, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-52940r3_rule" { 
                #Can't be checked
                $ActualStatus = "Open"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "MFR/AFNOAPPS$ and MHS$ come back in the results, looking into the trust accounts"
                }
                #V-8316
            #Windows Server 2012/2012 R2 Domain Controller Security Technical Implementation Guide :: Version 2, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            "SV-51175r3_rule" { 
                #Can't be checked
                $ActualStatus = "Open"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "REQ000000382837/This setting needs to be configured"
                }
                #V-8317
            #Windows Server 2012/2012 R2 Domain Controller Security Technical Implementation Guide :: Version 2, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-51186r2_rule" { 
                #Can't be checked
                $ActualStatus = "Open"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "MFR/We resctrict access to ports 389, 636, 3268, and 3269 per the firewall"
                }
                #V-14831
            #Windows Server 2012/2012 R2 Domain Controller Security Technical Implementation Guide :: Version 2, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            "SV-51188r2_rule" { 
                #Can't be checked
                $ActualStatus = "NotAFinding"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "MaxConnIdleTime is set to 900"
                }
                #V-15488
            #Windows Server 2012/2012 R2 Domain Controller Security Technical Implementation Guide :: Version 2, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-53145r1_rule" { 
                #Can't be checked
                $ActualStatus = "NotAFinding"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "Already configured"
                }
                #V-21954
            #Windows Server 2012/2012 R2 Domain Controller Security Technical Implementation Guide :: Version 2, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-53039r4_rule" { 
                #Can't be checked
                $ActualStatus = "Open"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "***GET WITH IA FOR DOCUMENTATION*** Have MFR (26496-rfv) that we can mirror."
                }
                #V-33673
            #Windows Server 2012/2012 R2 Domain Controller Security Technical Implementation Guide :: Version 2, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-51578r2_rule" { 
                #Can't be checked
                $ActualStatus = "NotAFinding"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "Every admin account when it is created is automatically added to the BC_CAT_VI which prevents internet access on any device that it logs into"
                }
                #V-36658
            #Windows Server 2012/2012 R2 Domain Controller Security Technical Implementation Guide :: Version 2, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-51581r2_rule" { 
                #Can't be checked
                $ActualStatus = "NotAFinding"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "User-level informatin is backed up daily per our RMAD server"
                }
                #V-36734
            #Windows Server 2012/2012 R2 Domain Controller Security Technical Implementation Guide :: Version 2, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-52130r2_rule" { 
                #Can't be checked
                $ActualStatus = "NotAFinding"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "All backups are stored in a secure data center that requires badge access from personal that are granted access with a need to get into the room."
                }
                #V-40173
            #Windows Server 2012/2012 R2 Domain Controller Security Technical Implementation Guide :: Version 2, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            ...
            "SV-51187r2_rule" { 
                #Can't be checked
                $ActualStatus = "NotAFinding"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "This is not a finding, we prevent anonymous access to directory data of a non-public directory"
                }
                #V-81495
            #Windows Server 2012/2012 R2 Domain Controller Security Technical Implementation Guide :: Version 2, Release: 18 Benchmark Date: 25 Oct 2019
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
...
if ($SCAP -eq $true) {
    Out-File -InputObject $checklist.Innerxml -Encoding default "$env:USERPROFILE\desktop\Checklist_$env:computername`_$timestamp.ckl"
    }
else {
