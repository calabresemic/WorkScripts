...
8/7/2020 : Michael Calabrese (1468714589) - Added Server 2019 STIGs
#>
...
if ($SCAP -eq $true) {
    ...
    foreach($drive in $drives){
        "`nScanning $drive"
        if($drive -eq "C"){
        $filelist=Get-ChildItem -Recurse "${drive}:\*" -Include *.exe.config,*.p12,*.pfx,*.java,*.jpp,Acrobat.exe,AcroRd32.exe,chrome.exe,EXCEL.EXE,firefox.exe,FLTLDR.EXE,GROOVE.EXE,iexplore.exe,INFOPATH.EXE,java.exe,javaw.exe,javaws.exe,lync.exe,MSACCESS.EXE,MSPUB.EXE,OIS.EXE,OneDrive.exe,OUTLOOK.EXE,plugin-container.exe,POWERPNT.EXE,PPTVIEW.EXE,VISIO.EXE,VPREVIEW.EXE,WINWORD.EXE,wmplayer.exe,wordpad.exe -ErrorAction SilentlyContinue
        $ProcessMitigationList +=($filelist | where {($_ -notlike "*.exe.config") -and ($_ -notlike "*.p12") -and ($_ -notlike "*.pfx") -and ($_ -notlike "*.java") -and ($_ -notlike "*.jpp")}).Name
        $execonfiglist+=($filelist | where {$_ -like "*.exe.config"}).FullName
        $softcerts+=($filelist | where {($_ -like "*.p12") -or ($_ -like "*.pfx")}).FullName
        $javalist+=($filelist | where {($_ -like "*.java") -or ($_ -like "*.jpp")}).FullName
        "Complete"
        }
        else{
        $filelist=(Get-ChildItem -Recurse "${drive}:\*" -Include *.exe.config,*.p12,*.pfx,*.java,*.jpp -ErrorAction SilentlyContinue).Name
        $execonfiglist+=($filelist | where {$_ -like "*.exe.config"}).FullName
        $softcerts+=($filelist | where {($_ -like "*.p12") -or ($_ -like "*.pfx")}).FullName
        $javalist+=($filelist | where {($_ -like "*.java") -or ($_ -like "*.jpp")}).FullName
        "Complete"
...
"@.Split("`n") | foreach {$_.trim()}
           ...
           #V-92987
            ...
            "SV-103079r1_rule" { 
                ...
                #Check to see if there's any local volumes that AREN'T compliant
                #if (Get-WMIObject -Class Win32_Volume | Where {$_.DriveType -eq 3 -and ($_.FileSystem -ne "NTFS" -and $_.FileSystem -ne "ReFS")}) {$ActualStatus = "Open"}
                if ([System.IO.DriveInfo]::getdrives() | Where-Object {$_.driveformat -ne "NTFS" -and $_.driveformat -ne "ReFS"}) {$ActualStatus = "Open"}
                else {$ActualStatus = "NotAFinding"}
                }
