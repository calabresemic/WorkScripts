...
$auditpolrepair="Run the audit policy repair script on the server CYOD\scripts\audit policy repair.ps1"
...
if ($SCAP -eq $true) {
    ...
    #This scans all drives for certain files
    #Added check for soft certs and java
    ...
    Start-Job -Name $drive -ScriptBlock {$drive=$args[0..($args.count -1)];(Get-ChildItem -Recurse "${drive}:\*" -Include *.p12,*.pfx,*.java,*.jpp,Acrobat.exe,AcroRd32.exe,chrome.exe,EXCEL.EXE,firefox.exe,FLTLDR.EXE,GROOVE.EXE,iexplore.exe,INFOPATH.EXE,java.exe,javaw.exe,javaws.exe,lync.exe,MSACCESS.EXE,MSPUB.EXE,OIS.EXE,OneDrive.exe,OUTLOOK.EXE,plugin-container.exe,POWERPNT.EXE,PPTVIEW.EXE,VISIO.EXE,VPREVIEW.EXE,WINWORD.EXE,wmplayer.exe,wordpad.exe -ErrorAction SilentlyContinue).FullName} -ArgumentList $drive | Out-Null
    }
    ...
    Write-Progress -Activity "Scanning the drives for the OS" -Status "Scanning the following drives: $scanningdrives" -PercentComplete $percent
    $runningjobs = (Get-Job -State Running).Count
    }
    ...
    $ProcessMitigationList=$allfiles | where {($_ -notlike "*.p12") -and ($_ -notlike "*.pfx") -and ($_ -notlike "*.java") -and ($_ -notlike "*.jpp")}
    ...
    if($ServerRole -eq 2){
        ...
        $ALLDNSZones=Get-DnsServerZone | where {$_.ZoneName -ne "TrustAnchors"} #Trust Anchors doesn't show up in the GUI so it should not be counted for the STIG
...
"`nBeginning STIG checks"
    ...
    $GPResult = Get-Content "$env:USERPROFILE\Desktop\gpresult.htm" #For testing
    }
...
"@
            ...
            "SV-52881r3_rule" {
                ...
                if($ServerRole -eq 2){
                    ...
                    $NonDefaultShares = Get-SmbShare | where {$_.name -ne "ADMIN$" -and $_.name -ne "C$" -and $_.name -ne "D$" -and $_.name -ne "E$" -and $_.name -ne "F$" -and $_.name -ne "IPC$"}    
                    }
                
                if($NonDefaultShares.count -gt 0){
                    $title = "Non-Default Shares"
                    $message = "Are the following shares required on this server?`nExample: BaseName, LogonScripts, StartupScripts, Scripts, emie`n`n" + ($NonDefaultShares.Name -join "`n")
...
"@.Split("`n") | foreach {$_.trim()}
            ...
            #The Windows 2012 DNS Server must be configured to enforce authorized access to the corresponding private key.
            "SV-51569r1_rule" { 
                if($serverRole -eq 2){$defaultacl="O:LSG:LSD:AI(A;ID;FA;;;SY)(A;ID;FA;;;BA)(A;ID;FA;;;S-1-5-80-880578595-1860270145-482643319-2788375705-1540778122)"}
                else{$defaultacl="O:LSG:LSD:AI(A;ID;FA;;;S-1-5-80-880578595-1860270145-482643319-2788375705-1540778122)(A;ID;FA;;;SY)(A;ID;FA;;;BA)"}
