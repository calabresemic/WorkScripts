...
if ($SCAP -eq $true) {
    ...
    else{$Guest=Get-LocalUser | where SID -like "*501"
        $Admin=Get-LocalUser | where SID -like "*500"}
            ...
            "SV-51175r3_rule" { 
                ...
                if($accounts.count -gt 0){
                    foreach($account in $accounts){
                        if($account -notlike "*.adm" -and $account -notlike "*.ada" -and $account -notlike "*.adc" -and $account -notlike "*.add" -and $account -notlike "*.ade" -and $account -notlike "*.adf" -and $account -notlike "*.adw" -and $account -notlike "*.adx"){$ActualStatus = "Open"}
                        }
                    }
                if($accounts -match "S-\d-\d+-(\d+-){1,14}\d+"){$checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].FINDING_DETAILS = "Orphaned SIDs found in the ACL."}
                }
...
'BUILTIN\Server Operators Allow  ReadAndExecute, Synchronize')
                    $sysvolacl=(Get-Acl E:\Windows\SYSVOL).AccessToString.split("`n")
                    foreach($line in $sysvolacl){if($line -notin $defaultacl){$badacl += $line}}
...
'APPLICATION PACKAGE AUTHORITY\ALL RESTRICTED APPLICATION PACKAGES Allow  -1610612736')
                    ...
                    $programfilesacl=(Get-Acl 'C:\Program Files').AccessToString.split("`n")
                    $programfiles86acl=(Get-Acl 'C:\Program Files (x86)').AccessToString.split("`n")
                    foreach($line in $programfilesacl){if($line -notin $defaultacl){$badacl += $line}}
                    foreach($line in $programfiles86acl){if($line -notin $defaultacl){$badacl += $line}}
                    if($badacl.count -eq 0){$ActualStatus = "NotAFinding"}
                    else{$ActualStatus = "Open"}
                    }
                else{$ActualStatus = "Open"}
...
'BUILTIN\Users Allow  ReadAndExecute, Synchronize')
                    $Cacl=(Get-Acl C:\).AccessToString.split("`n")
                    foreach($line in $Cacl){if($line -notin $defaultacl){$badacl += $line}}
                    if($badacl.count -eq 0){$ActualStatus = "NotAFinding"}
                    else{$ActualStatus = "Open"}
                    }
                else{$ActualStatus = "Open"}
...
'APPLICATION PACKAGE AUTHORITY\ALL RESTRICTED APPLICATION PACKAGES Allow  ReadAndExecute, Synchronize',
'APPLICATION PACKAGE AUTHORITY\ALL RESTRICTED APPLICATION PACKAGES Allow  -1610612736')
                    $windowsacl=(Get-Acl C:\Windows).AccessToString.split("`n")
                    foreach($line in $windowsacl){if($line -notin $defaultacl){$badacl += $line}}
                    if($badacl.count -eq 0){$ActualStatus = "NotAFinding"}
                    else{$ActualStatus = "Open"}
                    }
                else{$ActualStatus = "Open"}
...
'APPLICATION PACKAGE AUTHORITY\ALL APPLICATION PACKAGES Allow  ReadAndExecute, Synchronize',
'APPLICATION PACKAGE AUTHORITY\ALL RESTRICTED APPLICATION PACKAGES Allow  ReadAndExecute, Synchronize')
                $eventvwrAcl =(Get-Acl C:\Windows\System32\eventvwr.exe).AccessToString.split("`n")
                foreach($line in $eventvwrAcl){if($line -notin $defaultacl){$badacl += $line}}
                if($badacl.count -eq 0){$ActualStatus = "NotAFinding"}
                else{$ActualStatus = "Open"}
                }
