...
else{$cklFile = Get-ChildItem 'C:\Users\1468714589A\OneDrive - United States Air Force\desktop' -Filter *.ckl | sort whencreated -Descending | select -First 1}
$cklFileName = $cklFile.name.split(".")[0]
$cklFilePath = $cklFile.fullname
[xml]$checklist = Get-Content $cklFilePath

if ($SCAP -eq $true -or $automate -eq $true) {
...
elseif($automate -eq $true){
    if(!(Test-Path "\\muhj-fs-001.acc.accroot.ds.af.smil.mil\CYOD\07--Cyber 365\05--Checklists\$($Quarter)\$($GeoCode)")){New-Item -ItemType Directory "\\muhj-fs-001.acc.accroot.ds.af.smil.mil\CYOD\07--Cyber 365\05--Checklists\$($Quarter)\$($GeoCode)"}
    Out-File -InputObject $checklist.Innerxml -Encoding default "\\muhj-fs-001.acc.accroot.ds.af.smil.mil\CYOD\07--Cyber 365\05--Checklists\$($Quarter)\$($GeoCode)\$env:computername.ckl"
    }
else {
