...
if ($SCAP -eq $true) {
    ...
    $ProcessMitigation = Get-ProcessMitigation
    }
    ...
    #8/7/2020 - Added checks for 2019 STIGs and converted to jobs
    "Scanning the drives for the OS & .NET STIGS"
    ...
    Start-Job -Name $drive -ScriptBlock {$drive=$args[0..($args.count -1)];(Get-ChildItem -Recurse "${drive}:\*" -Include *.exe.config,*.p12,*.pfx,*.java,*.jpp,Acrobat.exe,AcroRd32.exe,chrome.exe,EXCEL.EXE,firefox.exe,FLTLDR.EXE,GROOVE.EXE,iexplore.exe,INFOPATH.EXE,java.exe,javaw.exe,javaws.exe,lync.exe,MSACCESS.EXE,MSPUB.EXE,OIS.EXE,OneDrive.exe,OUTLOOK.EXE,plugin-container.exe,POWERPNT.EXE,PPTVIEW.EXE,VISIO.EXE,VPREVIEW.EXE,WINWORD.EXE,wmplayer.exe,wordpad.exe -ErrorAction SilentlyContinue).FullName} -ArgumentList $drive | Out-Null
    }
    ...
    Write-Progress -Activity "Scanning the following drives: $scanningdrives" -Status "Progress: $percent%" -PercentComplete $percent
    $runningjobs = (Get-Job -State Running).Count
    }

    Write-Progress -Activity "Scanning the following drives: $scanningdrives" -Status "Ready" -Completed
    ...
    $ProcessMitigationList=$allfiles | where {($_ -notlike "*.exe.config") -and ($_ -notlike "*.p12") -and ($_ -notlike "*.pfx") -and ($_ -notlike "*.java") -and ($_ -notlike "*.jpp")}
    $execonfiglist=$allfiles | where {$_ -like "*.exe.config"}
...
"@.split("`n") | Foreach {$_.trim()}
            ...
            "SV-51175r3_rule" { 
                ...
                if($ServerRole -eq 3){$ActualStatus = "Not_Applicable"}
                    ...
                    foreach ($acl in $NTDSacl) {
                    switch ($acl.FileSystemRights) {
                        "FullControl" {
                            $accounts+=$acl.IdentityReference.Value | Where-Object {$_ -ne "NT AUTHORITY\SYSTEM" -and $_ -ne "BUILTIN\Administrators"}
                            }
                        }
                    }
                    if($accounts.count -gt 0){
                        foreach($account in $accounts){
                            if($account -notlike "*.adm" -and $account -notlike "*.ada" -and $account -notlike "*.adc" -and $account -notlike "*.add" -and $account -notlike "*.ade" -and $account -notlike "*.adef" -and $account -notlike "*.adw" -and $account -notlike "*.adx"){$ActualStatus = "Open"}
                            }
                        }
                    }
                }
...
"@.Split("`n") | foreach {$_.trim()}
            ...
            "SV-52136r3_rule" { 
                ...
                if ($Value -eq "0") {
                    ...
                    switch ($acl.IdentityReference) {
                        "CREATOR OWNER" {
                            $creatordefault = $acl.FileSystemRights -eq "FullControl" -and $acl.PropagationFlags -eq "InheritOnly"
                            }
                        "NT AUTHORITY\SYSTEM" {
                            $SYSTEMDefault = $acl.FileSystemRights -eq "FullControl"
                            }
           ...
           #V-58543
            ...
            "SV-73051r4_rule" { 
                ...
                $oldrecords=@()
                $dnszones=(Get-DnsServerZone | Where-Object {$_.IsReverseLookupZone -eq $false -and $_.ZoneName -ne "TrustAnchors"}).ZoneName
                foreach($dnszone in $dnszones){
                $CNames=@()
            ...
            "SV-73057r7_rule" { 
                ...
                $dnszones=(Get-DnsServerZone | Where-Object {$_.IsDsIntegrated -eq $true -and $_.ZoneName -ne "TrustAnchors"}).ZoneName
                $AAAA=@()
                foreach($dnszone in $dnszones){
                $AAAA+=Get-DnsServerResourceRecord -RRType AAAA -ZoneName $dnszone
                }
            ...
            #Both the log file and Event Tracing for Windows (ETW) for the IIS 8.5 web server must be enabled.
            "SV-91381r2_rule" { 
                $logname= "u_ex"+(Get-Date -Format yyMMdd)+".log"
$logdir= "C:\inetpub\logs\LogFiles\W3SVC1"
$log=Get-Content $logdir\$logname | where {$_ -notlike "#[D,S-V]*"}
...
if($proxies -contains $IISLog.cip){"Open"}
else{"NotAFinding"}
                }
            ...
            "SV-91391r5_rule" { 
                $defaultacl="O:BAG:SYD:AI(A;ID;FA;;;S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464)(A;OICIIOID;GA;;;S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464)(A;ID;FA;;;SY)(A;OICIIOID;GA;;;SY)(A;ID;FA;;;BA)(A;OICIIOID;GA;;;BA)(A;OICIIOID;GA;;;CO)"
            ...
            "SV-91423r4_rule" { 
                if($serversessionstate.cookieless -eq "UseCookies"){
                    if($serversessionstate.timeout -lt [timespan]"00:20"){$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                else {$ActualStatus = "Open"}
                }
            ...
            "SV-91441r2_rule" { 
                $defaultacl="O:SYG:SYD:PAI(A;OICIIO;GA;;;CO)(A;OICIIO;GA;;;SY)(A;;FA;;;SY)(A;OICIIO;GA;;;BA)(A;;FA;;;BA)(A;OICIIO;GXGR;;;BU)(A;;0x1200a9;;;BU)(A;OICIIO;GA;;;S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464)(A;;FA;;;S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464)"
            ...
            "SV-91445r1_rule" { 
                $defaultacl="O:S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464G:S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464D:PAI(A;;0x1200a9;;;SY)(A;;0x1200a9;;;BA)(A;;0x1200a9;;;BU)(A;;FA;;;S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464)(A;;0x1200a9;;;AC)(A;;0x1200a9;;;S-1-15-2-2)"
            ...
            #V-76775
            #IIS 8.5 Site Security Technical Implementation Guide :: Version 1, Release: 11 Benchmark Date: 24 Jul 2020
            ...
            "SV-91475r4_rule" { 
                if ((Get-WebConfiguration system.webServer/security/access -PSPath "IIS:\Sites\Default Web Site").sslFlags -contains "Ssl") {$ActualStatus = "NotAFinding"}
                else {$ActualStatus = "Open"}
                }
            ...
            "SV-91505r2_rule" { 
                if((Get-WebConfiguration system.webServer/security/access -PSPath "IIS:\Sites\Default Web Site").sslFlags -contains "SslRequireCert"){$ActualStatus = "NotAFinding"}
                else {$ActualStatus = "Open"}
                }
            ...
            "SV-91535r2_rule" { 
                ...
                if ($processModel.idleTimeout -le 20 -and $processModel.idleTimeout -gt 0) {$ActualStatus = "NotAFinding"}
                else {$ActualStatus = "Open"}
                }
            ...
            "SV-91577r1_rule" { 
                $failure=Get-ItemProperty IIS:\AppPools\$appPool -name failure
                if ($failure.rapidFailProtectionInterval -le 5) {$ActualStatus = "NotAFinding"}
                else {$ActualStatus = "Open"}
                }
           ...
           #V-92987
            ...
            "SV-103107r1_rule" { 
                $Value = Check-RegKeyValue "HKLM\System\CurrentControlSet\Control\Lsa\" "EveryoneIncludesAnonymous"
                if ($Value -eq "0") {
                    $defaultacl="O:S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464G:S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464D:PAI(A;OICIIO;GA;;;CO)(A;OICI;FA;;;SY)(A;OICI;FA;;;BA)(A;CI;LC;;;BU)(A;CIIO;DC;;;BU)(A;OICI;0x1200a9;;;BU)"
                    $Cacl=(Get-Acl C:\).Sddl
                    if($Cacl -eq $defaultacl){$ActualStatus = "NotAFinding"}
                    else{$ActualStatus = "Open"}
                    }
                else{$ActualStatus = "Open"}
            }
            ...
            "SV-103109r1_rule" { 
                $Value = Check-RegKeyValue "HKLM\System\CurrentControlSet\Control\Lsa\" "EveryoneIncludesAnonymous"
                if ($Value -eq "0") {
                    $defaultacl="O:S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464G:S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464D:PAI(A;OICIIO;GA;;;CO)(A;OICIIO;GA;;;SY)(A;;0x1301bf;;;SY)(A;OICIIO;GA;;;BA)(A;;0x1301bf;;;BA)(A;OICIIO;GXGR;;;BU)(A;;0x1200a9;;;BU)(A;CIIO;GA;;;S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464)(A;;FA;;;S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464)(A;;0x1200a9;;;AC)(A;OICIIO;GXGR;;;AC)(A;;0x1200a9;;;S-1-15-2-2)(A;OICIIO;GXGR;;;S-1-15-2-2)"
                    $programfilesacl=(Get-Acl 'C:\Program Files').Sddl
                    $programfiles86acl=(Get-Acl 'C:\Program Files (x86)').Sddl
                    if($programfilesacl -eq $defaultacl -and $programfiles86acl -eq $defaultacl){$ActualStatus = "NotAFinding"}
                    else{$ActualStatus = "Open"}
                    }
                else{$ActualStatus = "Open"}
                }
            ...
            "SV-103111r1_rule" { 
                ...
                if ($Value -eq "0") {
                    $defaultacl="O:S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464G:S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464D:PAI(A;OICIIO;GA;;;CO)(A;OICIIO;GA;;;SY)(A;;0x1301bf;;;SY)(A;OICIIO;GA;;;BA)(A;;0x1301bf;;;BA)(A;OICIIO;GXGR;;;BU)(A;;0x1200a9;;;BU)(A;CIIO;GA;;;S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464)(A;;FA;;;S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464)(A;;0x1200a9;;;AC)(A;OICIIO;GXGR;;;AC)(A;;0x1200a9;;;S-1-15-2-2)(A;OICIIO;GXGR;;;S-1-15-2-2)"
                    $windowsacl=(Get-Acl C:\Windows).Sddl
                    if($windowsacl -eq $defaultacl){$ActualStatus = "NotAFinding"}
                    else{$ActualStatus = "Open"}
                    }
                else{$ActualStatus = "Open"}
                }
            ...
            if($ServerRole -eq 3){$ActualStatus = "Not_Applicable"}
                ...
                $sysvolacl=(Get-Acl E:\Windows\SYSVOL).sddl
                $defaultacl="O:BAG:DUD:PAI(A;OICIIO;GA;;;CO)(A;OICIIO;GXGR;;;AU)(A;;0x1200a9;;;AU)(A;OICIIO;GA;;;SY)(A;;FA;;;SY)(A;OICIIO;GA;;;BA)(A;;0x1f01bf;;;BA)(A;OICIIO;GXGR;;;SO)(A;;0x1200a9;;;SO)"
                if($sysvolacl -eq $defaultacl){$ActualStatus = "NotAFinding"}
                else{$ActualStatus = "Open"}
                }
            }
            ...
            "SV-103127r1_rule" { 
                ...
                if($ServerRole -eq 3){$ActualStatus = "Not_Applicable"}
                    ...
                    $value = ($GPResult -match "<tr><td>Add workstations to domain").replace("<td>",";").split(";")[2].split("<")[0]
                    if ($Value -eq "Administrators") {$ActualStatus = "NotAFinding"}
                    elseif($domainName -eq "AFNOAPPS"){if($Value -eq "Administrators, Authenticated Users") {$ActualStatus = "NotAFinding"}}
                    else {$ActualStatus = "Open"}
                    }
                }
            ...
            "SV-103153r1_rule" { 
                ...
                $value = ($GPResult -match "<tr><td>Debug programs").replace("<td>",";").split(";")[2].split("<")[0]
                if ($Value -eq "Administrators") {$ActualStatus = "NotAFinding"}
                elseif ($Value -eq ""){$ActualStatus = "NotAFinding"}
                else {$ActualStatus = "Open"}
                }
            ...
            "SV-103159r1_rule" { 
                ...
                $ValArray = $value.Split(",") | foreach {$_.trim()}
                if ($ValArray.Count -eq 4 -and $valArray -contains "Administrators" -and $valArray -contains "Service" -and $valArray -contains "Local Service" -and $valArray -contains "Network Service") {$ActualStatus = "NotAFinding"}
                elseif($DomainName -eq "AFNOAPPS"){if($ValArray.Count -eq 4 -and $valArray -contains "Administrators" -and $valArray -contains "Service" -and $valArray -contains "Local Service" -and $valArray -contains "AFNOAPPS\IIS_WPG") {$ActualStatus = "NotAFinding"}}
                else {$ActualStatus = "Open"}
                }
...
"@
            ...
            "SV-103277r1_rule" { 
                $applicationACL=(Get-Acl C:\Windows\System32\winevt\Logs\Application.evtx).Sddl
                $defaultacl="O:LSG:LSD:AI(A;ID;FA;;;S-1-5-80-880578595-1860270145-482643319-2788375705-1540778122)(A;ID;FA;;;SY)(A;ID;FA;;;BA)"
                if($applicationACL -eq $defaultacl){$ActualStatus = "NotAFinding"}
                else{$ActualStatus = "Open"}
                }
            ...
            "SV-103279r1_rule" { 
                $securityACL=(Get-Acl C:\Windows\System32\winevt\Logs\Security.evtx).Sddl
                $defaultacl="O:LSG:LSD:AI(A;ID;FA;;;S-1-5-80-880578595-1860270145-482643319-2788375705-1540778122)(A;ID;FA;;;SY)(A;ID;FA;;;BA)"
                if($securityACL -eq $defaultacl){$ActualStatus = "NotAFinding"}
                else{$ActualStatus = "Open"}
                }
            ...
            "SV-103281r1_rule" { 
                $systemACL=(Get-Acl C:\Windows\System32\winevt\Logs\System.evtx).Sddl
                $defaultacl="O:LSG:LSD:AI(A;ID;FA;;;S-1-5-80-880578595-1860270145-482643319-2788375705-1540778122)(A;ID;FA;;;SY)(A;ID;FA;;;BA)"
                if($systemACL -eq $defaultacl){$ActualStatus = "NotAFinding"}
                else{$ActualStatus = "Open"}
                }
            ...
            "SV-103283r1_rule" { 
                #Checks that trusted installer is the only one with modify or full control
                $eventvwrAcl =(Get-Acl C:\Windows\System32\eventvwr.exe).sddl
                $defaultacl="O:S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464G:S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464D:PAI(A;;0x1200a9;;;SY)(A;;0x1200a9;;;BA)(A;;0x1200a9;;;BU)(A;;FA;;;S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464)(A;;0x1200a9;;;AC)(A;;0x1200a9;;;S-1-15-2-2)"
                if($eventvwrAcl -eq $defaultacl){$ActualStatus = "NotAFinding"}
                else{$ActualStatus = "Open"}
                }
            ...
            "SV-103317r1_rule" { 
                if(Confirm-SecureBootUEFI){$ActualStatus = "NotAFinding"}
                else{$ActualStatus = "Open"}
                }
            ...
            "SV-103427r1_rule" { 
                if(!($IsNIPR)){$ActualStatus = "Not_Applicable"}
                    ...
                    if($ProcessMitigationList -contains "java.exe"){
                        ...
                        if(($ProcessMitigation | where ProcessName eq "java.exe").ASLR.BottomUp -NE "ON"){$ActualStatus = "Open"}
