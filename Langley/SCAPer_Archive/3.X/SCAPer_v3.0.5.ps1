...
"@
            ...
            "SV-52863r2_rule" {
                ...
                if($ServerRole -eq 2){
                    $setting = ($Area52DefaultDomainPolicy -match "<tr><td>Password must meet complexity requirements")
                    if ($setting -ne "" -and $setting -ne $null) {
                        $value = $setting.replace("<td>",";").split(";")[2].split("<")[0]
            ...
            "SV-52893r1_rule" {
                ...
                if($ServerRole -eq 2){
                    $value = ($Area52DefaultDomainPolicy -match "<tr><td>Network security: Force logoff when logon hours expire")
...
"@.split("`n") | Foreach {$_.trim()}
            ...
            "SV-52938r2_rule" {
                ...
                if($ServerRole -eq 2){
                    $setting = ($Area52DefaultDomainPolicy -match "<tr><td>Minimum password length")
                    if ($setting -ne "" -and $setting -ne $null) {
                        $value = $setting.replace("<td>",";").split(";")[2].split("<")[0]
                        $num = [int]$value.Split(" ")[0]
                    if ($setting -ne "" -and $setting -ne $null) {
