...
if ($SCAP -eq $true) {
    ...
    "This is going to take a while especially if this is a DC"
    "Maybe imagine some elevator music"
    $drives=(Get-PSDrive -PSProvider FileSystem | Where-Object {$_.Used -ne 0}).name
    $execonfiglist=@()
    "Tis machine contains the following drives $drives"
    ...
    if ($SCAP -eq $true) {
        ...
        ###START Microsoft DotNet Framework 4.0 STIG###
            ...
            "SV-96209r2_rule" { 
                ...
                $value=Check-RegKeyValue "HKLM\SOFTWARE\WOW6432Node\Microsoft\.NETFramework\v4.0.30319" "SchUseStrongCrypto"
...
"@
            ...
            "SV-52855r1_rule" {
                ...
                elseif($ServerRole -eq 2){
                    if((Get-ADUser visitor).enabled -eq $false){$ActualStatus = "NotAFinding"}
                    else{$ActualStatus = "Open"}}
                }
            ...
            "SV-52856r1_rule" {
                ...
                if($ServerRole -eq 3){
                    if((Get-LocalUser Guest) -eq $true){$ActualStatus = "Open"}
                    else{$ActualStatus = "NotAFinding"}
                    }
                if($ServerRole -eq 2){
                    $ActualStatus = "Open"
            ...
            "SV-52857r1_rule" {
                ...
                if($ServerRole -eq 3){
                    if((Get-LocalUser Administrator) -eq $true){$ActualStatus = "Open"}
                    else{$ActualStatus = "NotAFinding"}
                    }
                if($ServerRole -eq 2){
                    $ActualStatus = "Open"
                    try{Get-ADUser Administrator}
                    catch{$ActualStatus = "NotAFinding"}
                    }
                }
            ...
            "SV-52218r2_rule" { 
                #Can't be checked
                $ActualStatus = "Open"
                $checklist.CHECKLIST.stigs.iSTIG.vuln[$STIG_index].COMMENTS = "This vulnerability currently has an MFR."
                }
...
"@.Split("`n") | foreach {$_.trim()}
            ...
            "SV-51188r2_rule" {
                ...
                $LDAPAdminLimits=dsquery * "cn=Default Query Policy,cn=Query-Policies,cn=Directory Service, cn=Windows NT,cn=Services,cn=Configuration,DC=AFNOAPPS,DC=USAF,DC=MIL" -attr LDAPAdminLimits
