    ...
    if($DSResults) { 
        ...
        foreach ($comp in $missingAssets) {
            ...
            if($sitecode.Length -eq 5) {
                ...
                Try{
                    ...
                    $missing+=[pscustomobject]@{'Asset Name'=$comp;'Asset Status'="Missing";'Service Account'=$svcacct.Name;'Lockout Status'=$svcacct.LockedOut;Enabled=$svcacct.Enabled;'Password Last Set'=$svcacct.PasswordLastSet}
                }
                Catch{
                    $missing+=[pscustomobject]@{'Asset Name'=$comp;'Asset Status'="Missing";'Service Account'=$_;'Lockout Status'='N/A';Enabled='N/A';'Password Last Set'='N/A'}
                }
        ...
        foreach ($comp in $BadAccess) {
            ...
            if($sitecode.Length -eq 5) {
                ...
                Try{
                    $svcacct=Get-ADUser $name -Properties Enabled,LockedOut,PasswordLastSet -Server afnoapps.usaf.mil -ErrorAction Stop
                    $missing+=[pscustomobject]@{'Asset Name'=$comp;'Asset Status'="Bad Access";'Service Account'=$svcacct.Name;'Lockout Status'=$svcacct.LockedOut;Enabled=$svcacct.Enabled;'Password Last Set'=$svcacct.PasswordLastSet}
                }
                Catch{
                    $missing+=[pscustomobject]@{'Asset Name'=$comp;'Asset Status'="Bad Access";'Service Account'=$_;'Lockout Status'='N/A';Enabled='N/A';'Password Last Set'='N/A'}
                }
